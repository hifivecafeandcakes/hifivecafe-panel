  ...
  const [result, setResult] = useState([])
    const handleFromDateChange = (e) => {
      ...
      const handleClick = (e) => {
        ...
        setLoading(true);
        axios.get(`${process.env.REACT_APP_API_URL}payment/birthday/list?fromdate=${fromDate}&todate=${lastDate}`)
          .then((res) => {
  ...
  const transformData = (data) => {
    ...
    return data.map((item, i) => {
      const renamedItem = {
        ...
        RESERVATION:(item.reservation_type=="BP" ? "Birthday Party" :""), 
        RESERVATION_TYPE:item.cat_title, 
        RESERVATION_CATEGORY:item.sub_tilte, 
        PRICE:item.sub_cat_price_range, 
        FINAL_BOOKING_AMT:item.final_amt, 
        PHOTO_HANGINGS:item.photo_hanging, 
        PHOTO_SHOOT:item.photo_shoot, 
        BOUGUET:item.bouquet, 
        FIRE_CRACKERS:item.fire_crackers, 
        BALLOON:item.balloon, 
        CAKE_WEIGHT:item.cake_weight, 
        CAKE_DESCRIPTION:item.cake_decription, 
        FLAVOUR:item.flavour,
        BOOKING_STATUS:(item.approval_status==1?"Approval":""), 
        PAYEMENT_STATUS:(item.payment_status== 1?"Paid":""), 
        BOOKING_DATE :(item.created_at.slice(0, 10).split('-').reverse().join('-')), 
        PAYMENT_DATE :(item.updated_at.slice(0, 10).split('-').reverse().join('-'))
      };
  ...
  const handleDownloadCSV = () => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        saveAs(blob, 'BIRTHDAY_BOOKING_WAITING.csv');
      }
    });
  };
    ...
    <>
        ...
        <div className="col-lg-12 my-3 px-3">
          <div className="row">
            <div className="col-lg-12">
              <div className="card">
                <div className="card-header">
                  <h3 className="card-title"> Birthday Payment List</h3> <Button className='btn bg-primary ml-5' style={{fontSize:"10px"}} onClick={handleDownloadCSV}>csv</Button>
                  <div className="card-tools">
                    <div
                      ...
                      <Table className="table table-hover text-nowrap">
                        <TableHead className='bg-warning'>
                          <TableRow style={{ fontSize: '15px' }} >
                            ...
                            <TableCell className='text-center'>FINAL BOOKING AMT</TableCell>
                            <TableCell className='text-center'>PHOTO HANGINGS</TableCell>
                            <TableCell className='text-center'>PHOTO SHOOT</TableCell>
                            <TableCell className='text-center'>BOUGUET</TableCell>
                            <TableCell className='text-center'>FIRE CRACKERS</TableCell>
                            <TableCell className='text-center'>BALLOON</TableCell>
                            <TableCell className='text-center'>CAKE WEIGHT</TableCell>
                            <TableCell className='text-center'>CAKE DESCRIPTION</TableCell>
                            <TableCell className='text-center'>FLAVOUR</TableCell>
                        ...
                        {result.map((item, index) => (
                              <TableRow key={index}>
                                ...
                                <TableCell className='text-center'>{item.reservation_type =="BP" ? "Birthday Party" :""}</TableCell>
                                ...
                                <TableCell className='text-center'>{item.final_amt}</TableCell>
                                <TableCell className='text-center'>{item.photo_hanging}</TableCell>
                                <TableCell className='text-center'>{item.photo_shoot}</TableCell>
                                <TableCell className='text-center'>{item.bouquet}</TableCell>
                                <TableCell className='text-center'>{item.fire_crackers}</TableCell>
                                <TableCell className='text-center'>{item.balloon}</TableCell>
                                <TableCell className='text-center'>{item.cake_weight}</TableCell>
                                <TableCell className='text-center'>{item.cake_decription}</TableCell>
                                <TableCell className='text-center'>{item.flavour}</TableCell>
                                <TableCell className='text-center'>{item.approval_status==1?"Approval":""}</TableCell>
                                <TableCell className='text-center'>
      ...
      >
        paid
                                ...
                                <TableCell className='text-center'>
                                    <div>
                                   { (item.updated_at.slice(0, 10).split('-').reverse().join('-'))}<br/>
...
export default BirthdayPaymentList