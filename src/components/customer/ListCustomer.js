...
const ListCustomer = () => {
  ...
  useEffect(() => {
    ...
    axios.get(`${process.env.REACT_APP_API_URL}/admin/customer/list`)
      .then((res) => {
        let data = res?.data;
                        ...
                        <Table className="table table-hover text-nowrap">
                          <TableHead className='bg-warning'>
                            <TableRow style={{ fontSize: '13px' }}>
                              ...
                              <TableCell className='text-center'>UPDATED AT</TableCell>
                            </TableRow>
                          </TableHead>
                          <TableBody>
                            {rows.map((item, index) => (
                              <TableRow key={index}>
                                ...
                                <TableCell className='text-center'>CUS{item.id}</TableCell>
                                <TableCell className='text-center'>{item.email}</TableCell>
                                <TableCell className='text-center'>{item.mobile}</TableCell>
                                <TableCell className='text-center' onClick={() => handleOpenModal1(item.profile_img)}>
                                ...
                                <TableCell className='text-center'>{(item.updated_at) ? (item.updated_at.slice(0, 10).split('-').reverse().join('-')) : (item.created_at.slice(0, 10).split('-').reverse().join('-'))}</TableCell>
                              </TableRow>
                            ))}
                          </TableBody>
                        </Table>
